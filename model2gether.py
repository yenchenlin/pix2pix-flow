...
def abstract_model_xy(sess, hps, feeds, train_iterator, test_iterator, data_init, lr, f_loss, domain):
    ...
    if hps.gradient_checkpointing == 1:
        ...
        gs = gradients(loss_train, params)
    else:
        gs = tf.gradients(loss_train, params)

    m.optimizer = optim.Optimizer()

    train_op, polyak_swap_op, ema = m.optimizer.adamax(
        params, gs, alpha=lr, hps=hps)
...
def model(sess, hps, train_iterator, test_iterator, data_init, domain):
